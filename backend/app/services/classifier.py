"""
LLM-based article category classifier.

Classifies each article into at most 2 categories from a fixed allowed list.
Falls back to None if the LLM is unavailable or confidence is too low.

Confidence thresholds (configurable in config.py or .env):
  CLASSIFIER_PRIMARY_MIN_CONFIDENCE   — primary dropped below this (default 0.6)
  CLASSIFIER_SECONDARY_MIN_CONFIDENCE — secondary dropped below this (default 0.5)
"""
...
If the article is ambiguous, add a "reason" field (max 1 sentence).
...
def classify_article(
    ...
    """
    Classify article using LLM into at most 2 categories.

    Returns ClassificationResult or None if LLM unavailable / low confidence.
    ...
    try:
        ...
        if result:
            log.debug(
                "classifier: '%s' → primary=%s(%.2f) secondary=%s(%.2f)",
                title[:60],
                result.primary,
